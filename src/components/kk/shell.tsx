...
import { ChevronLeft, MessageCircle, Mic, Package, ShoppingBag } from "lucide-react";
import type { ReactNode } from "react";
import { cn } from "@/lib/utils";
import { CameraModal } from "@/components/shreni/CameraModal";
import { ShreniAssistantPanel } from "@/components/shreni/ShreniAssistantPanel";
...
/** Phone-shaped viewport used by every KalaKart / ShreniKart screen. */
export function Phone({
  ...
  return (
    <div className="flex min-h-screen justify-center bg-beige">
      <div
        className={cn(
          "relative flex min-h-screen w-full max-w-[430px] flex-col bg-gradient-surface shadow-float overflow-x-hidden",
          className,
        )}
      >
        <PWAAppHeader />
        {withNav ? <PWAInstallBanner /> : null}
        <div className={cn("flex-1", withNav && "pb-32")}>{children}</div>
        {withNav ? (
          <>
            <BottomNav />
            {pathname !== "/voice" ? <ShreniVoiceOrb variant="floating" /> : null}
          </>
        ) : null}
        <CameraModal />
        <ShreniAssistantPanel />
      </div>
    </div>
  );
}
...
const tabs = [
  { to: "/dashboard", label: "Bazaar", icon: ShoppingBag },
  { to: "/voice", label: "Shreni AI", icon: Mic },
