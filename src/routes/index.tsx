...
import { PWAAppHeader } from "@/components/pwa/PWAAppHeader";
...
function AuthLanding() {
  ...
  return (
    <div className="flex min-h-screen justify-center bg-beige">
      <div className="relative flex min-h-screen w-full max-w-[430px] flex-col bg-gradient-surface shadow-float overflow-x-hidden">
        <PWAAppHeader />
        <div className="relative h-[40vh] min-h-[260px] w-full overflow-hidden">
          ...
          <div className="absolute bottom-5 left-5 right-5 rise">
            <span className="inline-flex items-center gap-1.5 rounded-full bg-gold/90 px-3 py-1 text-[11px] font-bold text-[#2b1608]">
              <Sparkles className="size-3.5" /> PWA App for Artisans
            </span>
            <h1 className="mt-2 font-display text-3xl font-bold leading-tight text-ivory">
              {t("brand")}
            </h1>
            <p className="mt-0.5 text-xs text-ivory/80">{t("tagline")}</p>
          </div>
        </div>
        ...
        <div className="flex flex-1 flex-col px-5 py-5">
          <div className="grid grid-cols-3 gap-2">
            {trust.map(({ icon: Icon, label }) => (
              <div
                key={label}
                className="rounded-2xl bg-card p-3 text-center shadow-soft border border-border/40"
              >
          ...
          <div className="mt-auto space-y-2.5 pt-6">
            ...
            <button
              ...
              onClick={async () => {
                ...
                await supabase.auth.signInWithPassword({ email: "9876543210@artisan.craftlink.app", password: "password" });
                void navigate({ to: "/dashboard", replace: true });
              }}
              className="tap flex w-full items-center justify-center gap-2 rounded-2xl bg-gradient-warm py-3.5 text-sm font-bold text-primary-foreground shadow-card"
            >
              <Sparkles className="size-4" /> Explore Demo as Artisan
            </button>
            <div className="flex gap-2">
              <Link
                to="/login"
                className="tap flex flex-1 items-center justify-center rounded-2xl border border-primary/30 bg-card py-3 text-sm font-bold text-primary"
              >
                {t("login")}
              </Link>
              <Link
            ...
            <p className="pt-1 text-center text-[10px] leading-relaxed text-muted-foreground">
              ShreniKart is an installable PWA application. Add to your home screen for full offline voice & camera features.
            </p>
          </div>
        </div>
      </div>
