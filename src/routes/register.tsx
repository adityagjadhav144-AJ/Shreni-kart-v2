...
import { ArrowLeft, Globe, KeyRound, Loader2, Mail, Palette, Smartphone, UserRound } from "lucide-react";
import { toast } from "sonner";
import { supabase } from "@/integrations/supabase/client";
import { isValidMobile, mobileToAccountEmail, normalizeMobile, useAuth } from "@/lib/auth";
import { LanguageSwitch, INDIAN_LANGUAGES, useI18n } from "@/lib/i18n";
...
function RegisterPage() {
  const { t, lang, setLang, openLanguageModal, currentLanguage } = useI18n();
            ...
            <div>
              <div className="mb-1.5 flex items-center justify-between">
                ...
                <button
                  ...
                  className="tap text-xs font-bold text-primary hover:underline flex items-center gap-1"
                >
                  <Globe className="size-3" />
                  <span>24 भाषाएँ ({currentLanguage.native})</span>
                </button>
              </div>
              <div className="grid grid-cols-3 gap-2">
                {INDIAN_LANGUAGES.filter((l) => l.popular).slice(0, 6).map((l) => (
                  <button
                    key={l.code}
                    type="button"
                    onClick={() => setLang(l.code)}
                    className={cn(
                      "tap rounded-2xl py-2.5 px-2 text-center text-xs font-semibold shadow-soft border transition-all",
                      lang === l.code
                        ? "bg-gradient-warm text-primary-foreground border-transparent ring-2 ring-primary/40"
                        : "bg-card border-border/50 hover:bg-card/80 text-foreground",
                    )}
                  >
                    <div className="font-display font-bold">{l.native}</div>
                    <div className="text-[10px] opacity-80 truncate">{l.label}</div>
                  </button>
                ))}
              </div>
            </div>
